...
describe('restoreTailnetRequestUrl', () => {
  it('restores the public base after a path-scoped proxy strips it', () => {
    ...
    expect(restoreTailnetRequestUrl('/@vite/client', '/graphtv')).toBe('/graphtv/@vite/client')
    expect(restoreTailnetRequestUrl('/src/main.js?q=1', '/graphtv')).toBe('/graphtv/src/main.js?q=1')
  })

  it('does not duplicate an intact public base', () => {
    expect(restoreTailnetRequestUrl('/graphtv', '/graphtv')).toBe('/graphtv/')
    expect(restoreTailnetRequestUrl('/graphtv?q=1', '/graphtv')).toBe('/graphtv/?q=1')
    expect(restoreTailnetRequestUrl('/graphtv/src/main.js', '/graphtv')).toBe('/graphtv/src/main.js')
  })
