...
describe('createChart', () => {
  ...
  it('selects season trendlines from their chart labels', () => {
    ...
    const labels = Array.from(container.querySelectorAll('.season-label'))
    const secondSeason = labels.find(
      (label) => label.__data__.seasonNumber === 2
    )

    expect(secondSeason.getAttribute('role')).toBe('button')
    expect(secondSeason.getAttribute('tabindex')).toBe('0')
    expect(secondSeason.getAttribute('aria-pressed')).toBe('false')
    expect(secondSeason.getAttribute('aria-label')).toBe(
      'Select Season 2 trendline'
    )
    secondSeason.dispatchEvent(new MouseEvent('click', { bubbles: true }))
    ...
    expect(secondSeason.getAttribute('aria-pressed')).toBe('true')
    expect(secondSeason.getAttribute('aria-label')).toBe(
      'Season 2 trendline selected'
    )

    const firstSeason = labels.find(
      (label) => label.__data__.seasonNumber === 1
    )
    firstSeason.dispatchEvent(
      new KeyboardEvent('keydown', { bubbles: true, key: ' ' })
    )
    expect(chart.getDebugState().selectedTrendId).toBe('season:1')

    secondSeason.dispatchEvent(
  ...
  it('does not expose season labels without a trendline as controls', () => {
    ...
    document.body.appendChild(container)
    const seasons = createTwoSeasons({ secondEpisodeCount: 2 })

    chart = createChart(container, seasons)
    const secondSeason = Array.from(
      container.querySelectorAll('.season-label')
    ...
    secondSeason.dispatchEvent(new MouseEvent('click', { bubbles: true }))
    expect(chart.getDebugState().selectedTrendId).not.toBe('season:2')
  })

  it('suppresses a season-label click generated by a touch pan', () => {
    vi.stubGlobal(
      'matchMedia',
      vi.fn((query) => ({
        matches:
    ...
    const selectionBeforeClick = chart.getDebugState().selectedTrendId

    secondSeason.dispatchEvent(new MouseEvent('click', { bubbles: true }))
    expect(chart.getDebugState().selectedTrendId).toBe(selectionBeforeClick)
  ...
  it('moves focus to the plot when a focused season label leaves the viewport', () => {
    const container = document.createElement('div')
    Object.defineProperty(container, 'clientWidth', {
      configurable: true,
      value: 600
