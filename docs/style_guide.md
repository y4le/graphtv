...
- Stack exactly two charts in reading order inside one visually cohesive duplex canvas and use direct show labels. Place the shared comparison context after both lanes so readers encounter all primary evidence before detail; it replaces the resting full-series detail, and episode or trend interaction temporarily uses the same pane.
...
- When a shared rating source is available, each lane may carry the companion show's non-interactive full-series trend and downward season-boundary ticks as summarized context. Never duplicate the companion episode points or hit targets, and label the context encoding directly beside the lane identity.
