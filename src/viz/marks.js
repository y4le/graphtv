...
export function renderSeasonLabels(
  svg,
  spans,
  viewport,
  scales,
  ...
  const labelLayer = svg
    .selectAll('.season-label-layer')
    .data([null])
    .join('g')
    .attr('class', 'season-label-layer')

  const focusedLabel = svg.node()?.ownerDocument.activeElement
  const focusedSeasonNumber = focusedLabel?.matches?.('.season-label')
    ? focusedLabel.__data__?.seasonNumber
    : null
  if (
    focusedSeasonNumber != null &&
    !labels.some((span) => span.seasonNumber === focusedSeasonNumber)
  ) {
  ...
  labelLayer
    ...
    .attr('font-size', 12)
    .attr('role', (span) =>
      isSelectableSeasonLabel(span, interactions) ? 'button' : null
    )
    .attr('tabindex', (span) =>
      isSelectableSeasonLabel(span, interactions) ? 0 : null
    )
    .attr('aria-label', (span) =>
      isSelectableSeasonLabel(span, interactions)
        ? getSeasonLabelAriaLabel(span, interactions)
        : null
    )
    ...
    .style('cursor', (span) =>
      isSelectableSeasonLabel(span, interactions) ? 'pointer' : null
    )
    .on('click', (event, span) => {
      if (!isSelectableSeasonLabel(span, interactions)) {
        return
      }
...
function isSelectableSeasonLabel(span, interactions) {
  return interactions.isSelectable?.(span.seasonNumber) ?? false
}

function getSeasonLabelAriaLabel(span, interactions) {
